...
import { CreateUserButton, ResetPasswordButton, DeleteUserButton, ChangeRoleSelect, EditUserButton } from '@/components/admin/AdminCreateUser';

type Profile = {
  id: string; full_name: string | null; avatar_url: string | null;
  email: string | null; role: string; parque: string | null;
  created_at: string; password_changed: boolean | null;
...
export default async function UsuariosPage() {
  ...
  const { data: pending } = await admin
    ...
    .eq('role', 'pending').order('created_at', { ascending: false });

  const { data: active } = await admin
    .from('profiles')
    .select('id,full_name,avatar_url,email,role,parque,created_at,password_changed,phone,department')
    .neq('role', 'pending').order('role').order('full_name');

  const total  = active?.length ?? 0;
  const agents = (active ?? []).filter(u => u.role === 'agent').length;
  const staff  = (active ?? []).filter(u => ['staff','admin'].includes(u.role)).length;
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Staff / Admin', value: staff,             icon: CheckCircle,color: '#7C3AED', bg: '#F5F3FF' },
          ...
          <div className="bg-amber-50 border-2 border-amber-200 rounded-2xl p-1">
            ...
            <div className="space-y-2 p-2">
              {(pending as { id: string; full_name: string | null; avatar_url: string | null; created_at: string }[])
                .map(u => <AdminApproveUser key={u.id} user={u} />)}
            </div>
          </div>
        </section>
      )}
      ...
      {/* Active users */}
      <section>
        <h2 className="text-xl font-black mb-4" style={{ color: '#0C2749' }}>Equipo activo ({total})</h2>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          {(active ?? []).map((user: Profile, i: number) => (
            <div key={user.id}
              className={`grid grid-cols-12 gap-2 px-5 py-4 items-center hover:bg-blue-50/30 transition-colors ${i !== 0 ? 'border-t border-gray-50' : ''}`}>
              <div className="col-span-4 flex items-center gap-3 min-w-0">
                {user.avatar_url
                  ...
                  : <div className="w-9 h-9 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0"
                      style={{ background: '#0C2749' }}>{user.full_name?.[0] ?? '?'}</div>
                }
                <div className="min-w-0">
                  <p className="font-semibold text-sm truncate" style={{ color: '#0C2749' }}>{user.full_name ?? 'Sin nombre'}</p>
                  {user.email && <p className="text-xs text-gray-400 truncate">{user.email}</p>}
                  {user.department && <p className="text-xs text-gray-300 truncate">{user.department}</p>}
                </div>
              </div>
